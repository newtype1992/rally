...
Hosted Supabase, production environment variables, PostHog, EAS, production Edge Functions, production push configuration, reminders, and social habit flows remain later lifecycle steps. The production `rally` URL scheme is retained; the missing iOS bundle identifier must be selected during EAS/App Store release configuration.
