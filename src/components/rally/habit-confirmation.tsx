...
import { ErrorState, OfflineNotice, RallyButton, RallySheetScreen, RallyText } from '@/components/rally/ui';
...
export function HabitConfirmation({ kind }: { kind: 'archive' | 'delete' }) {
  ...
  return <RallySheetScreen backgroundTitle={habitName ?? 'Habit'} sheetTitle={kind === 'archive' ? 'A little room to reset.' : 'Let this habit go?'} onClose={mutation.isPending ? undefined : close}>
    ...
    <RallyButton variant={kind === 'delete' ? 'danger' : 'primary'} disabled={!habitId || !online || !session} loading={mutation.isPending} onPress={submit}>{label}</RallyButton>
    <RallyButton variant="ghost" disabled={mutation.isPending} onPress={close}>Keep this habit</RallyButton>
